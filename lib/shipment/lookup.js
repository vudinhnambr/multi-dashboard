...
// "Use as Is" và "Repair Completed" được coi như "Closed" — nghĩa là đã xử lý xong,
// không còn vướng gì trước khi xuất hàng.
function classify(processingResults) {
  ...
  if (text.includes("closed") || text.includes("use as is") || text.includes("repair completed")) return "CLOSED";
  return "OPEN_REVIEW";
}
 ...
 */
// Một số ring S/N có đuôi nhà máy (vd "...VN"). Cho phép khớp hậu tố dù nhân viên gõ thiếu đuôi này.
const TRAILING_TAG_RE = /VN$/;
function suffixMatch(key, suffix) {
  return key.endsWith(suffix) || key.replace(TRAILING_TAG_RE, "").endsWith(suffix);
}
function resolveByPrefixAndSuffix(prefix, suffix, ringLookup) {
  ...
  for (const candidateKey of ringLookup.keys()) {
    if (candidateKey.startsWith(prefixUpper) && suffixMatch(candidateKey, suffixUpper)) {
...
function resolveAssySnKey(rawInput, ringLookup) {
  ...
  if (key.length >= MIN_SUFFIX_LENGTH) {
    const matches = [];
    for (const candidateKey of ringLookup.keys()) {
      if (suffixMatch(candidateKey, key)) {
        matches.push(candidateKey);
        if (matches.length > MAX_SUFFIX_CANDIDATES) break;
      }
    }
