...
function rowMinutes(r, reCheckCol, stdMap) {
  if (!r) return null;
  ...
  const stepStd = (stdMap || PART_COL_STD)[normPart(rawPart)];
...
export function aggregateCmmWeekly(rows, stdMap) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const rd = rowDateStr(r[0]);
    const eff = (rd && rd >= CMM_STD_CUTOVER) ? (stdMap || PART_COL_STD) : PART_COL_STD; // std theo mốc ngày
    const m = rowMinutes(r, reCheckCol, eff); if (m == null) continue;
...
export function aggregateCmmDaily(rows, dateStr, stdMap) {
  const header = rows[0] || [];
  let reCheckCol = -1;
  for (let i = 0; i < header.length; i++) {
    const h = String(header[i] || '').toLowerCase().replace(/\s+/g, ' ').trim();
  ...
  const byPart = {};
  const byStep = {}; // `${part}|||${stepLabel}` → { part, step, min, count }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const isITR = String(r[1] || '').trim().toUpperCase() === 'ITR';
    const stepNameRaw = String(r[4] || '').trim();
    const col = STEP_TO_COL[stepNameRaw.toLowerCase()];
    const eff = (dateStr >= CMM_STD_CUTOVER) ? (stdMap || PART_COL_STD) : PART_COL_STD; // std theo mốc ngày
    const stepStd = eff[normPart(rawPart)];
    const baseStd = (col !== undefined && stepStd && stepStd[col]) ? stepStd[col] : 0;
    const rc = reCheckCol >= 0 ? Number(r[reCheckCol]) : NaN;
    const reCheck = Number.isFinite(rc) && rc > 0 ? rc : 0;
    ...
    if (isITR) {
      ...
      rowMin = reCheck;
      const modelTxt = (col !== undefined && COL_LABEL[col]) ? COL_LABEL[col] : stepNameRaw;
      stepLabel = 'ITR' + (modelTxt ? ' - ' + modelTxt : '');
    } else {
      if (baseStd <= 0) continue;
      rowMin = baseStd + reCheck;
      stepLabel = COL_LABEL[col] || stepNameRaw || 'Step';
    }
    const part = getDisplayName(rawPart);
    byPart[part] = (byPart[part] || 0) + rowMin;
    const sk = part + '|||' + stepLabel;
    if (!byStep[sk]) byStep[sk] = { part, step: stepLabel, min: 0, count: 0 };
    byStep[sk].min += rowMin; byStep[sk].count += 1;
    totalMin += rowMin; count += 1;
  }
  const parts = Object.entries(byPart)
    .map(([part, m]) => ({ part, hours: Math.round(m / 60 * 10) / 10 }))
    .sort((a, b) => b.hours - a.hours);
  const partSteps = Object.values(byStep)
    .map(s => ({ part: s.part, step: s.step, hours: Math.round(s.min / 60 * 10) / 10, sets: s.count }))
    .sort((a, b) => b.hours - a.hours);
  return { hours: Math.round(totalMin / 60 * 10) / 10, count, parts, partSteps };
}
