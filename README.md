# Production Hub — gộp CMM + Auto MT

Một web duy nhất, một lần deploy Vercel, chứa cả 2 dashboard. Thanh menu trên
cùng để chuyển qua lại.

- **CMM Dashboard** — React (Vite), đọc dữ liệu trực tiếp từ Google Sheets.
  Không cần backend, không cần đăng nhập.
- **Auto MT Dashboard** — file HTML tĩnh (`public/auto-mt.html`), dùng Supabase
  (đọc/ghi/xóa) qua các serverless function trong `/api`, có đăng nhập Supabase Auth.
  Được nhúng vào khung chung bằng iframe nên chạy y hệt bản gốc.

## Cấu trúc

```
production-hub/
├── api/                  # Serverless functions của Auto MT (Vercel tự build)
│   ├── data.js           #   đọc production_records
│   ├── insert.js         #   ghi (inspector/admin)
│   └── delete.js         #   xóa (admin)
├── public/
│   ├── auto-mt.html      # Toàn bộ Auto MT dashboard (HTML/JS thuần)
│   └── favicon.svg
├── src/                  # Khung shell + CMM dashboard (React)
│   ├── pages/CMM.jsx      #   nội dung CMM
│   ├── App.jsx            #   top-nav + chuyển tab
│   ├── app-shell.css
│   ├── data.js, config.js, sampleData.js, dashboard.css, index.css
│   └── main.jsx
├── index.html
├── package.json
├── vite.config.js
└── vercel.json
```

## Chạy local

```bash
npm install
npm run dev
```

Lưu ý: khi chạy `npm run dev` thuần Vite, tab Auto MT mở `/auto-mt.html` được,
nhưng các API `/api/*` (Supabase) **chỉ chạy trên Vercel**. Muốn test API ở local
thì dùng `vercel dev` (cài `npm i -g vercel`).

## Deploy lên Vercel

1. Đẩy thư mục này lên một repo GitHub mới.
2. Vào Vercel → New Project → import repo đó. Framework để **Vite** (đã có sẵn
   trong `vercel.json`).
3. Khai báo Environment Variables (dùng cho `/api`, chính là 2 biến Auto MT đang dùng):
   - `SUPABASE_URL`
   - `SUPABASE_ANON_KEY`
4. Deploy.

### Điều hướng sau khi deploy

- `/`            → mở thẳng vào CMM (mặc định).
- `/#cmm`        → CMM.
- `/#auto-mt`    → Auto MT (iframe tới `/auto-mt.html`).
- `/api/data`, `/api/insert`, `/api/delete` → giữ nguyên như cũ.

## Cấu hình nguồn dữ liệu

- **CMM**: sửa `GSHEET_ID` trong `src/config.js`.
- **Auto MT**: `SUPABASE_URL` / `SUPABASE_ANON_KEY` đang hardcode trong
  `public/auto-mt.html` (phần `<script>` đầu file) — giống bản gốc. Các API server
  đọc từ Environment Variables trên Vercel.

## Còn để làm sau (login chung cho cả hai)

Hiện Auto MT có đăng nhập Supabase Auth, CMM thì mở tự do. Khi muốn gộp đăng nhập:
chuyển CMM sang gọi `/api/data` (Supabase) thay vì Google Sheets, rồi bọc cả khung
shell bằng một lớp kiểm tra session Supabase ở `App.jsx`. Đây là việc lớn hơn,
nên tách riêng.
